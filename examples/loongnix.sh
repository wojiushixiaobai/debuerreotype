...
initArgs=(
	--arch "$dpkgArch"
	--non-debian
	--exclude 'loongnix-gpu-driver-service,loonggpu-compiler'
)
...
if [ -d "/workdir" ]; then
	if [ ! -f "/workdir/$suite" ]; then
		ln -svL /usr/share/debootstrap/scripts/stable "/workdir/$suite"
	fi
fi
...
if [ ! -f "$rootfsDir/etc/apt/trusted.gpg" ]; then
	wget -O "$rootfsDir/etc/apt/trusted.gpg" https://github.com/wojiushixiaobai/docker-loongnix-artifacts/raw/master/debian/buster-slim/trusted.gpg
fi

tee "$rootfsDir/usr/lib/os-release" <<-EOS
	...
	BUG_REPORT_URL="https://www.loongnix.cn/"
EOS

# setup "proper" sources.list
tee "$rootfsDir/etc/apt/sources.list" <<-EOS
	deb $mirror $suite main
	deb http://download.jumpserver.org/debian buster main
EOS
